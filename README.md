...
**Your own two cards are arranged as a held pair.** gpokr overlaps them, and where they overlap a seat panel our canvas loses — it sits at z-index 9, beneath GWT's panels, so a slab drawn under one is simply gone where the site's own `<img>` was on top of it. Position is the one thing we can win with, so the pair is laid out here: the site's own spacing is kept when it's readable, opened to a minimum of a third of a card when it isn't, and the pair is pushed toward the middle of the felt *until it clears* any panel — pushed rather than nudged by a fixed amount, because how far it must go depends on how deep under the panel it started. A pair already in the open never moves. Each card sits slightly in front of the one to its left, which is what makes the overlap read as a stack rather than as one card with a strip beside it.
...
Each card also drops a **contact shadow** on the felt — card-shaped rather than the coin's radial blob, offset down and right because the light is up and left. Tight, deliberately: 4px of blur and about a pixel of offset. The first attempt was blurred 9px and offset up to 11px, which is the shadow of a card held an inch above the table, and it read exactly like that. There is no shadow map: nothing in this scene could catch one, since the felt belongs to the page underneath.
