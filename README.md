...
Dealt cards are **lit**, and on their own render layer so they can be lit differently from the chips without disturbing them — three decides what a light touches by intersecting layers, and the chip rig (ambient 1.35 plus a 1.5 key) is aimed at a glossy disc and would blow a sheet of paper out. Their key light comes from the same direction as the 3D table's (`KEY_POS`, upper-left toward the viewer), so the two canvases agree about where the light is. The intensities are solved rather than picked: a Lambert surface renders at `ambient + key · max(0, n·l)`, and a card at the default thickness has `n·l ≈ 0.57`, so `0.66 + 0.6 × 0.57 = 1.00` — the face renders at exactly its own colour and the artwork reads true. Everything else falls out for free: the underside rim faces away from the light and gets ambient alone (34% darker), the left rim catches a little more than the right, and a card's face slides down that curve as it turns over.

**Your own two cards stay exactly where gpokr puts them** and are stacked by depth: each card to the right is lifted by exactly one thickness, so the pair stacks the way two real cards do — the lower card's top face is where the upper card's underside begins. Without that they sat at the same depth, where two slabs sort arbitrarily and interpenetrate wherever they overlap, so the pair read as one card with a strip beside it. (An earlier version spread the pair and pushed it onto the felt to clear a seat panel. It cleared the panel and moved the cards a long way from where they belong, which was worse; a hand belongs at its seat.)

They get the same treatment as the board, without the animation: a hand card is *placed* rather than dealt, because its face is already on screen and covering it to fly it back in would hide the two cards you're deciding on, every hand. Which cards those are comes from the log's "pocket cards" line — the same kind of authority the board comes from, so a hand card is identified rather than guessed at. Other players' cards stay flat: they live inside their seat panels, which is what excludes them.

Cards cast **real shadows** — three's own shadow maps, from the geometry, not a painted sprite. That needs something to receive them, and nothing in this scene can: the felt is the *page*, underneath the canvas. `ShadowMaterial` is the way round it — a plane that draws only what is shadowed and stays transparent everywhere else, so an invisible catcher laid at the cards' underside puts their shadows straight over whatever the site drew down there. Cards then shadow each other for nothing, since it's the same map.

Two details decide whether it looks like contact or like floating. The catcher sits at the underside of the lowest card rather than below it — the further a receiving surface is from the caster, the further the light throws the shadow sideways. And the shadow camera is fitted to the cards actually on screen rather than to the viewport: a directional light's shadow map is an orthographic box, and one stretched over 1700px of page spends its 2048 texels at barely one per pixel, which turns a card's edge into a grey smear.
