...
Each card also drops a **soft shadow** on the felt — a card-shaped blurred quad rather than the coin's radial blob, offset down and right because the light is up and left, sliding further out and darkening as the card thickens. There is no shadow map: nothing in this scene could catch one, since the felt belongs to the page underneath.
