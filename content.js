(function () {
    ...
    // Your two cards, arranged as a hand you are holding rather than two slots the
    // site happened to lay out.
    //
    // gpokr overlaps them, and where they overlap a player panel our canvas
    // loses: it sits at z-index 9, under GWT's own panels, so a slab drawn under
    // one is simply gone — which is how a hole card ends up invisible when the
    // site's own <img> was on top of that panel and ours is not. Position is the
    // one thing we can win with, so the pair is laid out ourselves: overlapped
    // like a held hand, and nudged toward the middle of the felt, which is the
    // only direction guaranteed to be clear of the furniture round the rail.
    //
